...
@Component({
    selector: 'page-nearmiss',
    templateUrl: 'nearmiss.html'
})


export class NearmissPage implements OnInit {
    detailsOpen: boolean = true;
    conditionOpen: boolean = false;
    descriptionOpen: boolean = false;
    personalDetailsOpen: boolean = false;
    incidentTypeChoices = this.incidentFormService.nearmissChoices;
    ...
    submitIncident = () => {
        if (this.validateForm()) {
            ...
            this.dataService.submitIncidentData("nearmiss", incidentForm)
                .subscribe(data => {
